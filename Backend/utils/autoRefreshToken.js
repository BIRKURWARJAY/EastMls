...
export default async function autoRefreshToken(req, res) {
  ...
  try {
    ...
    if (!decodedToken) {
      return res.status(401).clearCookie("refreshToken").json({ message: "Unauthorized" });
    }
    ...
    if (token !== user?.refreshToken) {
      return res.status(401).clearCookie("refreshToken").json({ meassge: "Invalid Refresh Token" });
    }
