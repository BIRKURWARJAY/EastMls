...
function SellerEnquiry({ data }: SellerProps): ReactNode {
  ...
  const [inquiry, setinqury] = useState<typeof defaultInquiry>(defaultInquiry);
  ...
  const handleInquiry = async (): Promise<void> => {
    ...
    try {
      const response: Axios.AxiosXHR<any> = await api.post("/inquiry", inquiry);
      setinqury((prev) => ({
        ...prev,
        name: "",
        phone: "",
  ...
  return (
    ...
    >
      ...
      <TextField
        value={inquiry.name}
        onChange={(e) =>
          setinqury((prev) => ({ ...prev, name: e.target.value }))
        }
        variant="outlined"
        size="small"
        sx={{ bgcolor: "white" }}
      ...
      <TextField
        value={inquiry.phone}
        onChange={(e) =>
          setinqury((prev) => ({ ...prev, phone: e.target.value }))
        }
        variant="outlined"
        size="small"
        sx={{ bgcolor: "white" }}
      ...
      <TextField
        value={inquiry.email}
        onChange={(e) =>
          setinqury((prev) => ({ ...prev, email: e.target.value }))
        }
        variant="outlined"
        size="small"
        sx={{ bgcolor: "white" }}
      ...
      <TextareaAutosize
        ...
        onChange={(e) =>
          setinqury((prev) => ({ ...prev, message: e.target.value }))
        }
        minRows={3}
        style={{
